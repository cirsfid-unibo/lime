...
<xsl:stylesheet 
    ...
    xmlns:akn="http://docs.oasis-open.org/legaldocml/ns/akn/3.0/CSD09"
	...
	<xsl:template mode="elementAttributes" match="@*" >
    	<xsl:choose> 
    		<xsl:when test="not(starts-with(name(.),'xml'))">
    			...
    			<xsl:if test="name(.) = 'eId' or name(.) = 'eid' ">
	    				<xsl:attribute name="akn_currentId">
		    				<xsl:value-of select="." />
		    			</xsl:attribute>
		            </xsl:if> 
    		</xsl:when>
	...
	<xsl:template mode="undefinedElementAttributes" match="@*" >
			<xsl:choose>
	    		<xsl:when test="name(.) = 'eId' or name(.) = 'eid' or name(.) = 'href' or name(.) = 'showAs' or name(.) = 'showas'">
	    			<xsl:variable name="attName" select="concat('akn_',name(.))"/>
	    			<xsl:attribute name="{$attName}">
	    				<xsl:value-of select="." />
	    			</xsl:attribute>
