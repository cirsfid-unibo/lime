...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   xmlns:akn="http://www.akomantoso.org/2.0" exclude-result-prefixes="akn"
   version="2.0">
...
<xsl:if test="@style"><xsl:attribute name="style" select="@style"/></xsl:if>
   ...
   <xsl:template match="akn:img">
      ...
      <img>
         ...
         <xsl:attribute name="src" select="concat('http://www.senato.it/japp/bgt/showdoc/17/DDLPRES/700298/',@src)"/>
         <xsl:for-each select="@*">
            <xsl:variable name="attName" select="concat('data-',local-name(.))"/>
            <xsl:attribute name="{$attName}" select="."/>
         </xsl:for-each>
